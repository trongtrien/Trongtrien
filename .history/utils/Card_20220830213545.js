    ...
    return (
            <div className='rounded p-0 bg-secondary bg-opacity-25 border' style={{border: "solid 1px rgba(0,0,0,0.1)"}}>
                ...
                <div className='d-flex p-2 pt-3' style={{alignItems: "center"}}>
                    <Link href={data.link} passHref><span className='btn fs-4 btn-main-y rounded'>TIẾNG HÀN {data.title}</span>
                    <span className='btn ms-auto fs-5 text-dl'>$ <i className='text-success'>Free</i></span>
                </div>
                <hr className='p-0 m-0'/>
                <div className='p-2 m-0'>
                    <Link href={data.link} passHref><h4 className='btn fs-4 btn-main-y m-0 p-0 pb-2'>{data.title}</h4></Link>
