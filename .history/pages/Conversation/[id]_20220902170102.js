...
export async function getServerSideProps({params}) {
    try {
     ...
     const quizGrammar_data = await Axios({method: "GET", url: "/lessonsdata"})
     const datas = await quizGrammar_data.data
     return {props: {
        id: id,
        data: await quizGrammar_data.data.find(d => d.id == id)||null,
        quizGrammar_data: await quizGrammar_data.data
    }}
    } catch (error) {
...
const LanddingPage = ({id,data,quizGrammar_data}) => {
                            ...
                            <div className='p-1 pt-3'>
                                <div className='ratio ratio-16x9 m-0 border border-warning rounded'>
                                    <ReactPlayer
                                        width="100%"
                                        height="100%"
                                        url={`https://www.youtube.com/watch?v=${id=="05"?"7ICd0_CmnUg":data.con_yt_id}?start=5`}
                                        controls={true}
                                    />
                                </div>
                            </div>
