...
const SeachDic = ({ datas }) => {
        ...
        const search = () => {
          setNewquery(query)
          if(!newData) {setResult(<h2 className="mt-2">Từ bạn nhập không đúng</h2>)
                              ...
                              <div>
                                  <audio src={`/media/audio/u${newData.game_id}/${newData.audio}`} width="750" height="500" controls controlsList="nodownload"></audio>
                              </div>
                          </div>
                          )}}
        ...
        return (
          <div className="bg-secondary rounded-3 p-3 mb-4">
            <div>
              ...
              />
              <button className="text-light" type="submit" onClick={()=>{
                search()
                if(newData){
                  setQuery('')
                }
