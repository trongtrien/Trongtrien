...
const Lesson = ({
  ...
  const lesson = lesson_ssr[0]
      ...
      const message = "Phần này chỉ dành cho học viên ghi danh vui lòng ghi danh để được sử dụng chức năng này"
        ...
        {lesson?<>
          <div className='col-xl-9 p-0 m-0 border-1 border-end border-warning'>
                    ...
                    {token?(idprogress===0?(<h4 className='text-logo-b text-center pt-5'> 
                            ...
                            <h4 className='text-logo-b text-center pt-5'></h4>:
                            <Player 
                    ...
                    <ul className={`nav nav-tabs pt-2 ${!isMobile800&&'ps-5'}`} style={{fontSize: "15px"}}>
                        ...
                        <li className="nav-item item-action ms-0">
                            <button id='btn4' disabled={token=="undefined"} onClick={()=>setBtnId('4')}  className={`nav-link p-2 ${btnId==="4"&&"active"}`}>Ghi chú</button>
                        </li>
                        <li className={`nav-item item-action pt-2 ms-auto ${isMobile800?'m-0 ps-0 pe-0':'ps-5 pe-5'}`} onClick={() => setShowShare(!showShare)}>
                    ...
                    <div className='p-3 mt-1'>
                        ...
                        {btnId==="1"&&<>
                        {token?<div>Comment here</div>:<h5>{message}</h5>}
                        </>}
