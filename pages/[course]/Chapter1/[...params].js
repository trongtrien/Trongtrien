...
export async function getServerSideProps({params}) {
   try {
    ...
    const resp = await Axios({method: "get", url: `/lesson1/${id}`})
    const lessons = await Axios({method: "get", url: 'lesson1'})
    const courseviews = await Axios({method: "get", url: '/courseviews/1'})
    const dic = await Axios({method: "GET", url: `/gamesQ/${id}`})
    const progress = await Axios({method: "GET", url: `/progress`})
    const resp1 = await Axios({method: "get", url: '/question/1'})
    return {
      props: {
...
//  progress
const progresses = progress.filter(p => p.course_id == 1).find(p => p.userId == decoded.userId)
  ...
  return (
    <CourseLayout link={link}
      ...
      <SEO title={lesson.title}
           description='Khóa học tiếng Hàn EPS-TOPIK 1. Chúng tôi tâm huyết - các bạn nỗ lực, thành công sẽ đến là chuyện không ảo tưởng'
